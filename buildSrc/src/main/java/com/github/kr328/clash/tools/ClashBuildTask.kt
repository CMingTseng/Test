...
abstract class ClashBuildTask : DefaultTask() {
    ...
    fun build() {
        ...
        println("Show go build inputDirectory $input " )
        val output = outputDirectory.file
        println("Show go build output $output " )
        val config = config.get()
        println("Show go build config $config " )
        val environment = Environment(ndkDirectory.file, config.minSdkVersion)
        println("Show go build environment $environment " )
